...
# Serialize CartItem: current_cart[product_id] = serializers.serialize("json", [line_item])
...
def deserialize_cart(cart):
    return [next(serializers.deserialize("json",item)).object for item in cart.values()]
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if same_billing_address:
                ...
                self.request.session["billing_address"] = serialize_object(billing_address)

            self.request.session["shipping_address"] = serialize_object(shipping_address) 
            self.request.session.modified = True
...
class process_order(View):
    def post(self, *args, **kwargs):
        ...
        # shipping_address => {"id": null, "address1": "202-07 56th Ave.", "address2": "", "country": "US", "state": "NY", "city": "Oakland Gardens", "zipcode": "11364", "billingaddress_ptr": null}

        
        contact_info = deserialize_object(self.request.session["contact"])
        shipping_info = deserialize_object(self.request.session["shipping_address"])
        billing_info = deserialize_object(self.request.session["billing_address"])
